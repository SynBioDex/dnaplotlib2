...
class GlyphRenderer:
    ...
    def __init__(self, glyph_path=None):
        ...
        if glyph_path is None:
            ...
            self.glyphs_library, self.glyph_term_map = self.load_glyphs_from_path(
                glyph_path
            )
    ...
    def get_glyph_parameters(self, glyph_type=None, default_values=False):
        ...
        if glyph_type:
            try:
                ...
                warnings.warn(
                    f"""'{glyph_type}' is not a glyph type in the glyphs library."""
                )
        else:
    ...
    def __process_style(self, style_text):
        ...
        for el in style_els:
            key_val = [
                x.strip() for x in el.split(":")
            ]  # Convert style element into list
            if key_val[0] in self.svg2mpl_style_map.keys():
                # Create new key-value pair with processed value
                style_data[
                    self.svg2mpl_style_map[key_val[0]]
                ] = self.__process_unknown_val(key_val[1])
        return style_data
    ...
    def __eval_svg_data(svg_text, parameters):
        ...
        """
        return re.sub(
            r"{([^{}]+)}", lambda m: str(eval(m.group()[1:-1], parameters)), svg_text
        )
    ...
    def draw_glyph(
        ...
        # Collate parameters
        merged_parameters, label_parameters = collate_user_params(
            self, glyph_type, user_parameters
        )
        ...
        if user_style is not None:
            ...
            for key in user_style.keys():
                if key not in path_ids:
                    warnings.warn(
                        f"""'{key}' is not a valid path ID for the '{glyph_type}' glyph."""
                    )
        paths_to_draw = []
        path_zorders = None
        ...
        for path in glyph["paths"]:
            if path["class"] not in ["baseline", "bounding-box"]:
                ...
                if (
                    user_style is not None
                    and path["id"] is not None
                    and path["id"] in user_style.keys()
                ):
                    ...
                    for style_el in merged_style.copy():
                        ...
                        if style_el not in path["style"].keys():
                            ...
                            warnings.warn(
                                f"""Style parameter '{style_el}' is not valid for '{path["id"]}'."""
                            )
        ...
        all_y_flipped_paths = []
        position = adjust_position_for_orientation(
            position, orientation, merged_parameters["width"], rotation
        )
        for path in paths_to_draw:
            path_index = paths_to_draw.index(path)
            y_flipped_path = self.__flip_position_rotate_glyph(
                path[0], baseline_y, position, orientation, rotation
            )
            all_y_flipped_paths.append([y_flipped_path])
            patch = patches.PathPatch(
                y_flipped_path, **path[1], zorder=zorders_to_use[path_index]
            )
            if ax is not None:
                ax.add_patch(patch)
        if user_parameters is not None:
            if label_parameters is not None:
                # Draw label
                processed_label_params = self.process_label_params(
                    label_parameters, all_y_flipped_paths
                )
                ax.text(**processed_label_params, ha="center", va="center")
    ...
    def process_label_params(self, label_parameters, paths):
        ...
        textpos_x, textpos_y = self.calculate_centroid_of_paths(
            all_path_vertices, xy_skew=xy_skew
        )
        return {
            "x": textpos_x,
            "y": textpos_y,
    ...
    def get_glyph_bounds(
        self, glyph_type, position, rotation=0.0, user_parameters=None
    ):
    ...
    def get_baseline_end(
        self, glyph_type, position, orientation, rotation=0.0, user_parameters=None
    ):
        ...
        if baseline_path is not None:
            ...
            baseline_y = glyph["defaults"]["baseline_y"]
            y_flipped_path = self.__flip_position_rotate_glyph(
                baseline_path, baseline_y, position, orientation, rotation
            )
...
class Construct:
    ...
    def draw(self, draw_for_bounds=False):
        ...
        if draw_for_bounds is False:
            fig, ax, baseline_start, baseline_end, bounds = render_part_list(
                self.part_list,
                self.renderer,
    ...
    for part in part_list:
        ...
        if user_parameters is not None:
            if "vertical_offset" in user_parameters:
                ...
                part_position = (
                    part_position[0]
                    + (user_parameters["vertical_offset"]) * sin(bearing),
                    part_position[1]
                    + (user_parameters["vertical_offset"]) * cos(bearing),
                )
        # Draw the part
        bounds, part_position = renderer.draw_glyph(
            ax,
            part[0],
            part_position,
        ...
        if user_parameters is not None:
            ...
            if "trailing_gap_skew" in user_parameters:
                ...
                part_position = (
                    part_position[0] + trailing_gap_skew * cos(rotation),
                    (part_position[1] + trailing_gap_skew * sin(rotation)),
                )
        if part is not part_list[-1]:
            part_position = (
                part_position[0] + gapsize * cos(rotation),
    ...
    if interaction_list is not None:
        ...
        for interaction in interaction_list:
            if interaction[2] in interaction_types:
                ...
                warnings.warn(
                    f"""'{interaction[2]}' is not a valid interaction type."""
                )
...
def adjust_position_for_orientation(position, orientation, glyph_width, rotation):
    ...
    """
    if orientation == "reverse":
        position = (position[0] + glyph_width * cos(rotation)), (
            position[1] + glyph_width * sin(rotation)
        )
    return position
...
def collate_user_params(renderer, glyph_type, user_parameters):
    ...
    if user_parameters is not None:
        ...
        for key in user_parameters.keys():
            if (
                key not in glyph["defaults"]
                and key != "label_parameters"
                and key != "orientation"
                and key != "vertical_offset"
                and key != "trailing_gap_skew"
                and key != "path_zorders"
            ):
...
def draw_interaction(
    ax, sending_bounds, receiving_bounds, interaction_type, parameters, rotation=0.0
):
    ...
    ):  # Degradation is bigger than the other interactions
        initial_distance = parameters["distance_from_baseline"] * 2
    # Determine pad size
    y_pad = parameters["heightskew"] * 2
    # Find centroid of glyph bounds
    origin_cent = (
        sending_bounds[0][0] + (sending_bounds[1][0] - sending_bounds[0][0]) / 2,
        sending_bounds[0][1] + (sending_bounds[1][1] - sending_bounds[0][1]) / 2,
    )
    end_cent = (
        receiving_bounds[0][0] + (receiving_bounds[1][0] - receiving_bounds[0][0]) / 2,
    ...
    int_origin_y = origin_cent[1] + (
        initial_distance + parameters["distance_from_baseline"]
    ) * cos(bearing * pi / 180)
    # Determine origin max
    int_origin_max = (
        int_origin_x
        + (y_pad + parameters["sending_length_skew"]) * sin(bearing * pi / 180),
        int_origin_y
        + (y_pad + parameters["sending_length_skew"]) * cos(bearing * pi / 180),
    )
    # Determine end max
    int_end_max = (
        int_origin_max[0] + centroid_distance * sin((90 - rotation) * pi / 180),
        int_origin_max[1] + centroid_distance * cos((90 - rotation) * pi / 180),
    )
    # Determine interaction endpoint
    int_end_x = int_end_max[0] + (y_pad + parameters["receiving_length_skew"]) * sin(
        (180 - rotation) * pi / 180
    )
    int_end_y = int_end_max[1] + (y_pad + parameters["receiving_length_skew"]) * cos(
        (180 - rotation) * pi / 180
    )
    if parameters["direction"] == "reverse":
    ...
    plt.plot(
        [int_origin_x, int_origin_max[0], int_end_max[0], int_end_x],
        [int_origin_y, int_origin_max[1], int_end_max[1], int_end_y],
        color=parameters["color"],
        lw=parameters["linewidth"],
        zorder=parameters["zorder"] - 5,
    )  # Slightly lower zorder than head to prevent overlap
    # Control
...
def draw_control(ax, int_end_x, int_end_y, parameters, rotation=0.0):
    ...
    point1 = (
        int_end_x + (parameters["headwidth"] / 2) * sin(bearing1 * pi / 180),
        int_end_y + (parameters["headwidth"] / 2) * cos(bearing1 * pi / 180),
    )
    point2 = (
        point1[0] + (parameters["headwidth"] / 2) * sin(bearing2 * pi / 180),
        point1[1] + (parameters["headwidth"] / 2) * cos(bearing2 * pi / 180),
    )
    point3 = (
        int_end_x + (parameters["headwidth"] / 2) * sin(bearing2 * pi / 180),
        int_end_y + (parameters["headwidth"] / 2) * cos(bearing2 * pi / 180),
    )
    # Draw
    plt.plot(
        [int_end_x, point1[0], point2[0], point3[0], int_end_x],
        [int_end_y, point1[1], point2[1], point3[1], int_end_y],
        color=parameters["color"],
        lw=parameters["linewidth"],
...
def draw_degradation(ax, int_end_x, int_end_y, parameters, rotation=0.0):
    ...
    point = (
        int_end_x + (parameters["headheight"]) * sin(bearing3 * pi / 180),
    ...
    origin = (
        int_end_x
        + (parameters["headheight"] * 2 + parameters["headwidth"] / 2)
        * sin(bearing3 * pi / 180),
        int_end_y
        + (parameters["headheight"] * 2 + parameters["headwidth"] / 2)
        * cos(bearing3 * pi / 180),
    ...
    end1 = (
        origin[0] + (parameters["headwidth"] / 2) * sin(bearing1 * pi / 180),
        origin[1] + (parameters["headwidth"] / 2) * cos(bearing1 * pi / 180),
    )
    end2 = (
        origin[0] + (parameters["headwidth"] / 2) * sin(bearing2 * pi / 180),
